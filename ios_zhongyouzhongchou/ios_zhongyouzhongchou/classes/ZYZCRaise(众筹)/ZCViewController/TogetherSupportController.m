//
//  TogetherSupportController.m
//  ios_zhongyouzhongchou
//
//  Created by liuliang on 16/7/11.
//  Copyright © 2016年 liuliang. All rights reserved.
//
#define KTOGETHER_SUPPORT(rate,money)  [NSString stringWithFormat:@"一起去:支持%d％旅费(%.2f元)",rate,money]

#define KLIMIT_TEXT(limitNum,totalNum) [NSString stringWithFormat:@"限额：%@位    |    已报名：%ld位",limitNum,totalNum]

#import "TogetherSupportController.h"
#import "ZCDetailCustomButton.h"
#import "MBProgressHUD+MJ.h"
#import "WXApiManager.h"
@interface TogetherSupportController ()<UIScrollViewDelegate>
...
@implementation TogetherSupportController
...
-(void)configUI
{
    ...
    //描述
    _descLab=[[UILabel alloc]initWithFrame:CGRectMake(KEDGE_DISTANCE, _titlelab.bottom+KEDGE_DISTANCE, _bgImg.width-2*KEDGE_DISTANCE, 0)];
...
-(void)createBottomBtn
{
    ...
    [sureBtn setTitle:@"报名一起去" forState:UIControlStateNormal];
    [sureBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    sureBtn.layer.cornerRadius=KCORNERRADIUS;
    sureBtn.layer.masksToBounds=YES;
    sureBtn.backgroundColor=[UIColor ZYZC_MainColor];
    [sureBtn addTarget:self action:@selector(supportTogther) forControlEvents:UIControlEventTouchUpInside];
...
-(void)setTogetherUersModel:(TogetherUersModel *)togetherUersModel
{
    ...
    _titlelab.text=KTOGETHER_SUPPORT(rate,[togetherUersModel.price floatValue]/100.0);
    
    //描述文字
    NSString *support_together_text=
    ZYLocalizedString(@"skim_support_together");
    
    CGFloat textHeight=[ZYZCTool calculateStrByLineSpace:10.0 andString:support_together_text andFont:[UIFont systemFontOfSize:15] andMaxWidth:_descLab.width].height;
...
//    }
    _descLab.attributedText=[ZYZCTool setLineDistenceInText:support_together_text];
    _descLab.numberOfLines=0;
    _descLab.height=textHeight;
    
    //限额
    _limitLab.text=KLIMIT_TEXT(togetherUersModel.sumPeople,togetherUersModel.users.count);
    ...
    for (int i=0; i<togetherUersModel.users.count; i++) {
        ZCDetailCustomButton *btn=[[ZCDetailCustomButton alloc]initWithFrame:CGRectMake((edg+btnWidth)*(i%num), (edg+btnWidth)*(i/num), btnWidth, btnWidth)];
        UserModel *partnerModel=togetherUersModel.users[i];
        btn.userId=partnerModel.userId;
        lastBtnBottom=btn.bottom;
        [btn sd_setImageWithURL:[NSURL URLWithString:partnerModel.img?partnerModel.img:partnerModel.faceImg] forState:UIControlStateNormal];
...
-(void)supportTogther
{
    ...
    BOOL hasSupportTogther=NO;
    for (UserModel *user in _togetherUersModel.users) {
        if ([userId isEqual:user.userId]) {
            hasSupportTogther=YES;
            break;
        }
    }
    if (hasSupportTogther) {
