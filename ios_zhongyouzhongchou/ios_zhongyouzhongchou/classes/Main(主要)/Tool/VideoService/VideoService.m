...
//获取帧图片
+ (NSArray *) thumbnailImagesForVideo:(NSURL *)videoURL withImageCount:(NSInteger ) count
{
    NSMutableArray *thumbnailImages=[NSMutableArray array];
    CGFloat timeLen=[VideoService getVideoDuration:videoURL];
    DDLog(@"timeLen:%.2f",timeLen);
    AVURLAsset *asset = [[AVURLAsset alloc] initWithURL:videoURL options:nil];
    NSParameterAssert(asset);
    AVAssetImageGenerator *assetImageGenerator =[[AVAssetImageGenerator alloc] initWithAsset:asset];
    assetImageGenerator.appliesPreferredTrackTransform = YES;
    ...
    for (NSInteger i=0; i<20; i++) {
        ...
        }else
        {
            UIImage *thumbImage = [[UIImage alloc] initWithCGImage:thumbnailImageRef];
            CGImageRelease(thumbnailImageRef);
            [thumbnailImages addObject:thumbImage];
        }
    }
    if (!thumbnailImages.count) {
        return nil;
    }
    else
    {
