...
- (void)_setupPlayer {
    ...
    [[XMNPhotoManager sharedManager] getVideoInfoWithAsset:self.asset.asset completionBlock:^(AVPlayerItem *playerItem, NSDictionary *playetItemInfo) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if (playerItem) {
                self.selectedVideoEnable=YES;
            }
            else
            {
                self.selectedVideoEnable=NO;
            }
            __weak typeof(*&self) self = wSelf;
...
- (void)_handlePlayAciton {
    ...
    if (!self.selectedVideoEnable) {
        UIAlertView *alert=[[UIAlertView alloc]initWithTitle:@"暂时无法播放" message:nil delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
        [alert show];
        return;
    }
